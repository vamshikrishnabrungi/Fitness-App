...
async def history(user_id: UUID = Depends(current_user_id), session: AsyncSession = Depends(get_session)) -> list[SessionView]:
    ...
    rows = (await session.scalars(select(TrainingSession).where(TrainingSession.athlete_id == athlete_id).order_by(TrainingSession.scheduled_for.desc()).limit(100))).all()
...
async def today(user_id: UUID = Depends(current_user_id), session: AsyncSession = Depends(get_session)) -> SessionView | None:
    ...
    row = await session.scalar(select(TrainingSession).where(TrainingSession.athlete_id == athlete_id, TrainingSession.scheduled_for >= now.replace(hour=0, minute=0, second=0), TrainingSession.scheduled_for < now.replace(hour=0, minute=0, second=0) + __import__('datetime').timedelta(days=1)).order_by(TrainingSession.scheduled_for))
...
async def get_session_detail(session_id: UUID, user_id: UUID = Depends(current_user_id), session: AsyncSession = Depends(get_session)) -> SessionView:
    ...
    if row is None: raise ProblemError(404, "session_not_found", "Session not found", "The session does not exist.")
