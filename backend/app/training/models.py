...
class TrainingPlan(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "plans"
    __table_args__ = (UniqueConstraint("athlete_id", "plan_number", name="uq_athlete_plan_number"), {"schema": "training"})
...
class SessionItem(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    slot_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("knowledge.recipe_slots.id"), nullable=False)
    sequence: Mapped[int] = mapped_column(Integer, nullable=False)
