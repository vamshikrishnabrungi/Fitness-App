...
async def encrypt_json(payload: dict[str, Any], *, aad: bytes) -> Envelope:
    ...
    if settings.kms_key_name:
        ...
        raise RuntimeError("KMS_KEY_NAME is required for sensitive production data")
    else:
        local_key = hashlib.sha256(settings.local_envelope_key.encode()).digest()
        local_nonce = os.urandom(12)
        wrapped = local_nonce + AESGCM(local_key).encrypt(local_nonce, dek, aad)
        key_version = "local-development-v1"
...
async def decrypt_json(envelope: Envelope, *, aad: bytes) -> dict[str, Any]:
    ...
    if envelope.key_version == "local-development-v1":
        if settings.is_production:
            raise RuntimeError("Local development ciphertext cannot be decrypted in production")
        local_key = hashlib.sha256(settings.local_envelope_key.encode()).digest()
