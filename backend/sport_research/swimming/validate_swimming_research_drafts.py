...
def _validate_metadata(data: Dict[str, Any]) -> List[str]:
    ...
    if metadata.get("sport") != "swimming":
        errors.append("metadata.sport must be swimming")
...
def validate(paths: Iterable[str]) -> int:
    ...
    if not draft_paths:
        print("No swimming draft files found.")
        return 0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate swimming sport research draft JSON files.")
