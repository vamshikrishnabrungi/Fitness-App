...
SWIMMING_DIR = Path(__file__).resolve().parent
DRAFT_DIR = SWIMMING_DIR / "drafts"
OUTPUT_PATH = SWIMMING_DIR / "SWIMMING_RESEARCH_DRAFT_SUMMARY.md"
...
def _render_domain(path: Path, data: Dict[str, Any]) -> List[str]:
    ...
    lines = [
        f"## {domain}",
        "",
        f"- File: `{path.relative_to(SWIMMING_DIR)}`",
...
def build_summary(paths: Iterable[Path]) -> str:
    ...
    lines = [
        "# Swimming Research Draft Summary",
        "",
        "This file is generated from structured swimming research drafts. It is a review index, not the final sport database.",
        "",
...
def main() -> None:
    ...
    if not draft_paths:
        raise SystemExit("No swimming draft files found.")
