...
OUTPUT_PATH = CRICKET_DIR / "CRICKET_RESEARCH_DRAFT_SUMMARY.md"
...
def _item_title(item: Any) -> str:
    if isinstance(item, dict):
        for key in ("name", "title", "id", "rule", "concept", "model", "quality"):
            value = item.get(key)
...
def _render_domain(path: Path, data: Dict[str, Any]) -> List[str]:
    lines = [
        f"## {data.get('domain') or path.stem}",
        "",
        f"- File: `{path.relative_to(CRICKET_DIR)}`",
        f"- Summary: {str(data.get('executive_summary') or '').strip()}",
    ]
...
def main() -> None:
    ...
    if not draft_paths:
        raise SystemExit("No cricket draft files found.")
