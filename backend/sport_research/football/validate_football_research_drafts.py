...
def _validate_metadata(data: Dict[str, Any]) -> List[str]:
    ...
    if metadata.get("sport") not in {"soccer", "football"}:
        errors.append("metadata.sport must be soccer or football")
...
def validate(paths: Iterable[str]) -> int:
    ...
    if not draft_paths:
        print("No football draft files found.")
        return 0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate football sport research draft JSON files.")
