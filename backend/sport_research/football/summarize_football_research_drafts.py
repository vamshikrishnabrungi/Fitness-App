...
OUTPUT_PATH = FOOTBALL_DIR / "FOOTBALL_RESEARCH_DRAFT_SUMMARY.md"
...
def _render_domain(path: Path, data: Dict[str, Any]) -> List[str]:
    ...
    lines = [
        f"## {domain}",
        "",
        f"- File: `{path.relative_to(FOOTBALL_DIR)}`",
...
def build_summary(paths: Iterable[Path]) -> str:
    ...
    lines = [
        "# Football Research Draft Summary",
        "",
        "This file is generated from the structured football research drafts. It is a review index, not the final sport database.",
        "",
...
def main() -> None:
    ...
    if not draft_paths:
        raise SystemExit("No football draft files found.")
