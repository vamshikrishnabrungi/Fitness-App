...
BASKETBALL_DIR = Path(__file__).resolve().parent
DRAFT_DIR = BASKETBALL_DIR / "drafts"
OUTPUT_PATH = BASKETBALL_DIR / "BASKETBALL_RESEARCH_DRAFT_SUMMARY.md"
...
def _render_domain(path: Path, data: Dict[str, Any]) -> List[str]:
    ...
    lines = [
        f"## {domain}",
        "",
        f"- File: `{path.relative_to(BASKETBALL_DIR)}`",
...
def build_summary(paths: Iterable[Path]) -> str:
    ...
    lines = [
        "# Basketball Research Draft Summary",
        "",
        "This file is generated from the structured basketball research drafts. It is a review index, not the final sport database.",
        "",
...
def main() -> None:
    ...
    if not draft_paths:
        raise SystemExit("No basketball draft files found.")
