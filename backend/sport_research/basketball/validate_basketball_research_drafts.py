...
def _validate_metadata(data: Dict[str, Any]) -> List[str]:
    ...
    if metadata.get("sport") != "basketball":
        errors.append("metadata.sport must be basketball")
...
def validate(paths: Iterable[str]) -> int:
    ...
    if not draft_paths:
        print("No basketball draft files found.")
        return 0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate basketball sport research draft JSON files.")
