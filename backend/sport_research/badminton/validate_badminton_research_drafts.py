...
def _validate_metadata(data: Dict[str, Any]) -> List[str]:
    ...
    if metadata.get("sport") != "badminton":
        errors.append("metadata.sport must be badminton")
...
def validate_file(path: Path) -> List[str]:
    ...
    if source_count < 4:
        errors.append("source_refs should include at least 4 credible sources")
...
def validate(paths: Iterable[str]) -> int:
    ...
    if not draft_paths:
        print("No badminton draft files found.")
        return 0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate badminton sport research draft JSON files.")
