...
BOXING_DIR = Path(__file__).resolve().parent
DRAFT_DIR = BOXING_DIR / "drafts"
OUTPUT_PATH = BOXING_DIR / "BOXING_RESEARCH_DRAFT_SUMMARY.md"
...
def _render_domain(path: Path, data: Dict[str, Any]) -> List[str]:
    ...
    lines = [
        f"## {domain}",
        "",
        f"- File: `{path.relative_to(BOXING_DIR)}`",
...
def build_summary(paths: Iterable[Path]) -> str:
    ...
    lines = [
        "# Boxing Research Draft Summary",
        "",
        "This file is generated from the structured boxing research drafts. It is a review index, not the final sport database.",
        "",
...
def main() -> None:
    ...
    if not draft_paths:
        raise SystemExit("No boxing draft files found.")
