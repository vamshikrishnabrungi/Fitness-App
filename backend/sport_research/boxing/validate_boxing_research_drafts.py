...
def _validate_metadata(data: Dict[str, Any]) -> List[str]:
    ...
    if metadata.get("sport") != "boxing":
        errors.append("metadata.sport must be boxing")
...
def validate(paths: Iterable[str]) -> int:
    ...
    if not draft_paths:
        print("No boxing draft files found.")
        return 0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate boxing sport research draft JSON files.")
