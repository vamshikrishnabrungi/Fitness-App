...
VOLLEYBALL_DIR = Path(__file__).resolve().parent
DRAFT_DIR = VOLLEYBALL_DIR / "drafts"
OUTPUT_PATH = VOLLEYBALL_DIR / "VOLLEYBALL_RESEARCH_DRAFT_SUMMARY.md"
...
def _item_title(item: Any) -> str:
    if isinstance(item, dict):
        for key in ("name", "title", "id", "rule", "concept", "model", "quality", "record_type", "collection"):
            value = item.get(key)
...
def _render_domain(path: Path, data: Dict[str, Any]) -> List[str]:
    ...
    lines = [
        f"## {domain}",
        "",
        f"- File: `{path.relative_to(VOLLEYBALL_DIR)}`",
...
def build_summary(paths: Iterable[Path]) -> str:
    ...
    lines = [
        "# Volleyball Research Draft Summary",
        "",
        "This file is generated from the structured volleyball research drafts. It is a review index, not the final sport database.",
        "",
...
def main() -> None:
    ...
    if not draft_paths:
        raise SystemExit("No volleyball draft files found.")
