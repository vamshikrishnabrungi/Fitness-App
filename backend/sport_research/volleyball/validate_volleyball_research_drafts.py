...
def _validate_metadata(data: Dict[str, Any]) -> List[str]:
    ...
    if metadata.get("sport") != "volleyball":
        errors.append("metadata.sport must be volleyball")
...
def validate(paths: Iterable[str]) -> int:
    ...
    if not draft_paths:
        print("No volleyball draft files found.")
        return 0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate volleyball sport research draft JSON files.")
