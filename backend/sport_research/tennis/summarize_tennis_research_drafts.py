...
OUTPUT_PATH = TENNIS_DIR / "TENNIS_RESEARCH_DRAFT_SUMMARY.md"
...
def _render_domain(path: Path, data: Dict[str, Any]) -> List[str]:
    ...
    lines = [
        f"## {domain}",
        "",
        f"- File: `{path.relative_to(TENNIS_DIR)}`",
...
def build_summary(paths: Iterable[Path]) -> str:
    ...
    lines = [
        "# Tennis Research Draft Summary",
        "",
        "This file is generated from structured tennis research drafts. It is a review index, not the final sport database.",
        "",
...
def main() -> None:
    ...
    if not draft_paths:
        raise SystemExit("No tennis draft files found.")
