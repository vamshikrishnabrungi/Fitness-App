...
def _validate_metadata(data: Dict[str, Any]) -> List[str]:
    ...
    if metadata.get("sport") != "tennis":
        errors.append("metadata.sport must be tennis")
...
def validate(paths: Iterable[str]) -> int:
    ...
    if not draft_paths:
        print("No tennis draft files found.")
        return 0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate tennis sport research draft JSON files.")
