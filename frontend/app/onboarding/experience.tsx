...
export default function ExperienceScreen(){const router=useRouter();const insets=useSafeAreaInsets();const store=useOnboardingStore();
 ...
 const next=()=>{store.setBaseline({experience_level:level,runs_per_week:Number(runs)||0,weekly_distance:Number(weekly)||0,longest_recent_run:Number(longest)||0,training_interruption:interruption});router.push('/onboarding/profile');};
 return <View style={[coachLayout.container,{paddingTop:insets.top}]}><CoachProgress step={3}/><ScrollView style={coachLayout.scrollView} contentContainerStyle={coachLayout.content}><CoachCard icon="speedometer" eyebrow="CURRENT BASELINE" title="Where are you starting from?" subtitle="Use a normal recent week. Estimates are fine, and new runners can enter zero.">
